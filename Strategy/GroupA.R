...
# -- Global parameters (identical to GroupX.R) --
num_symbols <- 100
train_length <- 252
lookahead <- 21
retrain_freq <- 21
...
# ==========================================================
# Vectorized feature computation for ALL (date x symbol)
# ==========================================================
compute_all_features <- function(close_mat, vol_mat, open_mat) {
  n <- nrow(close_mat)
  m <- ncol(close_mat)
  n_feat <- 30  # base features before date/cross-sectional
  
  feat_3d <- array(NA_real_, dim = c(n, m, n_feat))
  
  # --- EMA-based: MACD, PPO, MACD_hist ---
  ...
  # --- RSI 14 ---
  delta <- close_mat[-1, , drop = FALSE] - close_mat[-n, , drop = FALSE]
  ...
  # --- Rolling SD via cumsum of squares ---
  cs_c2 <- apply(close_mat^2, 2, cumsum)
  ...
  # Realised volatility (rolling SD of log returns)
  log_ret <- matrix(0, n, m)
  ...
  # EMA state for incremental updates (last row values)
  ema_state <- list(
    ema12  = ema12[n, ],
    ema26  = ema26[n, ],
    signal = sig_ema[n, ],
...
# Feature index map (for the 31 base features in feat_3d)
FI <- list(rsi=1, bb_high=2, bb_low=3, MACD=4, PPO=5, MACD_hist=6,
           ...
           roc_5=28, roc_21=29, zscore_20=30, zscore_50=31)

# ==========================================================
# Build training data from feature array
# ==========================================================
build_training_data <- function(feat_3d, r21_fwd, date_strs) {
  n <- dim(feat_3d)[1]; m <- dim(feat_3d)[2]; nf <- dim(feat_3d)[3]
  
  # valid rows: have r21_fwd and enough warmup
  ...
  # Pre-allocate feature matrix: (nr * m) rows x (nf + 2 + 7) cols
  ...
  for (ri in seq_along(valid_rows)) {
    ...
    # Base features (31 columns)
    for (fi in 1:nf) feat_mat[idx, fi] <- feat_3d[i, , fi]
    ...
    # Cross-sectional deciles: r01(14), r05(15), r10(16), r21(17), r42(18), r63(19)
    ret_cols <- c(14, 15, 16, 17, 18, 19)
    ...
    # dollar_vol_rank
    vals <- feat_mat[idx, FI$log_dollar_vol]
    feat_mat[idx, 40] <- rank(vals, na.last = "keep") / m
    
    # Labels
    label_vec[idx] <- r21_fwd[i, ]
...
# ==========================================================
# Extract features for a single day (row) from stored state
# ==========================================================
extract_today_features <- function(close_mat, vol_mat, open_mat, ema_st,
  ...
  # SMA from rolling sums stored in state
...
initialise_state <- function(data) {
  ...
  # Compute all features at once (vectorized)
...
trading_algorithm <- function(new_data, state) {
  ...
  close_mat <- close_mat[-1, , drop = FALSE]
  ...
  # Update wealth and positions drift
  if (day_idx > 1) {
  ...
  # Incrementally update EMA state (O(1) per symbol)
  a12 <- 2/13; a26 <- 2/27; a9 <- 2/10
  ...
  # Incrementally update RSI state
  ...
  # Retrain periodically (use vectorized batch feature computation)
  if (day_idx %% retrain_freq == 1 || is.null(model)) {
    res <- compute_all_features(close_mat, vol_mat, open_mat)
    td <- build_training_data(res$feat_3d, r21_fwd, rownames(close_mat))
    model <- if (!is.null(td)) train_lgb_fast(td$feat_mat, td$label_vec) else model
    # Also refresh EMA state from full computation (avoid drift)
    ema_state <- res$ema_state
  }
  
  # Extract today's features for all symbols at once
